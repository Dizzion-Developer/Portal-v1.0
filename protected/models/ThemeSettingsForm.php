...
class ThemeSettingsForm extends ThemeSettings {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('id, org_id, type, header, hover, link, created_by, created_dt, modified_by, modified_dt, button', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function deleteOrgThemeSettings($orgId) {
        ...
        if ($themesettings_mapped) {
            if (ThemeSettingsForm::model()->deleteAllByAttributes(array('org_id' => $orgId))) {
                echo $msg = "theme settings deleted successfully";
            } else {
               echo $msg = false;
            }
        } else {
           echo $msg = "Theme settings doesnot exists";
        }
        return $msg;
    }
